...
// it('list("apples") returns a thing', () => {
//   list("apples")
// });


it('list("apples") returns a thing where head is "apples"', () => {
  const l = list("apples");     // a list
  const hopefullyApples = l.head 
  expect(l.head).toBe("apples");
});
...
describe('map', () => {
  it('map(length-of-string, list("apples")) returns list(6)', () => {
    expect(map((s)=>s.length, list("apples"))).toEqual(list(6));
  });
